...
lazy_static! {
    static ref COID_INC: AtomicUsize = AtomicUsize::new(0);
     ...
     static ref COLS: HashMap<String, usize> = {
        ...
        // map.insert("f_0_4".to_string(), 1);
        // map.insert("f_4_0".to_string(), 2);
        // map.insert("funding_rate_predict".to_string(), 3);
        map
    };
...
pub fn cb_filled(_tinfo: &mut TradeInfo, pitem: &PendingItem, filled_amount: f64) {
    info!(
        "cb_filled, fkey={} coid={} amount={}",
        pitem.from_key, pitem.client_order_id, pitem.amount
    );
    ...
    if pitem.client_order_id.starts_with("o") {
            ...
            if let Some(mut ongoing) = ONGOING_BID.try_write_for(Duration::from_secs(1)) {
                if ongoing.contains_key(&open_from_key.to_string()) {
                    ...
                    for ci in cvs.citems.iter_mut() {
                        ...
                        info!(
                            "fkey={} coid={} fillamount={} camount={} totalamount={}",
                            open_from_key,
        ...
        panic!(
            "from_key={} client_order_id={}",
            pitem.from_key, pitem.client_order_id
        );
    }
}
...
pub fn cb_finished(ts: i64, tinfo: &mut TradeInfo, pitem: &PendingItem, is_cancel: bool) {
    info!(
        "cb finished, fkey={} amount={} coid={} pitem.side={} is_cancel={}",
        pitem.from_key, pitem.amount, pitem.client_order_id, pitem.side, is_cancel
    );
    ...
    if pitem.client_order_id.starts_with("o") {
        ...
        //bid:1:0:0.0004
        let sidt = ps[1].parse::<i32>().unwrap();
        let sidm = ps[1].parse::<i32>().unwrap();
        let e_m = &ENGIN_CONF.sids[&sidm];
        let market_m = get_market()
            .get(
                (e_m["exchange"].to_string() + ":" + &e_m["etype"] + ":" + &tinfo.symbol_std)
                    .as_str(),
            )
            .unwrap();
        let _dinfo_m: &DepthInfo = tinfo.depths.get(&sidm).unwrap();
        let _pos_m: f64 = *tinfo.pos.get(&sidm).unwrap();
        ...
        if pitem.side == "bid" {
            ...
            if amount_deal != 0. {
                ...
                let start_close_ts = if PAIRMM_CONF.is_start_close_open {
                    pitem.create_ts / 1000
                } else if ts > 47157326100 {
                    ts / 1000
                ...
                let cvitem = VCITEM {
                    ...
                    total_amount: amount_deal,
                    start_close_ts: start_close_ts,
                    citems: cvitem,
                };
                ...
                // let cvitem = VCITEM{sidm:sidm,sidt:sidt, sidet:"ask".to_string(), from_key:pitem.from_key.to_string(), total_amount:amount_deal, start_close_ts:pitem.create_ts/1000, citems:cvitem};
...
pub fn is_cancel(t: &Vec<f64>, tinfo: &mut TradeInfo, pitem: &PendingItem, sid: &i32) -> bool {
    ...
    if pitem.client_order_id.starts_with("op_") {
        // println!("op======={}=={}",pitem.from_key,pitem.client_order_id);
        if (t[0] * 1000.) as i64 - pitem.create_ts
            >= (PAIRMM_CONF.max_open_order_keep_s * 1000) as i64
        {
            info!(
                "pending last too long cid={} o={} c={}",
                pitem.client_order_id, t[0] as i64, pitem.create_ts
            );
            return true;
        }
    }
    ...
    if pitem.from_key.starts_with("cl_") {
        ...
        if (t[0] * 1000.) as i64 - pitem.create_ts
            >= (PAIRMM_CONF.max_close_order_keep_s * 1000) as i64
        {
            info!(
                "pending last too long cid={} o={} c={}",
                pitem.client_order_id, t[0] as i64, pitem.create_ts
            ...
            for (_price, vpitem) in porders.phash.iter() {
                for pitem in vpitem.iter() {
                    if is_cancel(t, tinfo, pitem, &sid) {
                        let v: CancelDecision = CancelDecision {
                            sid: *sid,
                            side: pitem.side.to_string(),
                            price: pitem.price,
                            client_order_id: pitem.client_order_id.to_string(),
                            delayup_ms: msts + ENGIN_CONF.cancel_delay_ms,
                        };
                        cds.push(v);
                    }
                }
...
pub fn get_target_prem_by_ranges(
    _side: &str,
    ranges: Vec<f64>,
    _sf: f64,
    _is_open: bool,
...
pub fn get_range_of_pstat(_ts: i64, _side: &str, _sid: i32) -> Vec<f64> {
    //pstat
    //let k = record.get(1).unwrap().to_string() + ":" + &record.get(2).unwrap().to_string() + ":" + &record.get(3).unwrap().to_string();

    let vs: Vec<f64> = vec![0.0, 0.0002, 0.0005, 0.001];
    return vs;
}

pub fn get_oppo_close_ability(_ts: i64, _sid: i32) -> bool {
    return true;
}

pub fn get_rbs_from_df_maker_close(_t: &Vec<f64>, df: f64, side: &str) -> f64 {
    let mut rs = -1.;
    ...
    let target_thr_maker_close = 0.0002;
    if side == "buy" {
...
pub fn get_close_decision(t: &Vec<f64>, tinfo: &mut TradeInfo) -> Vec<MakeDecision> {
    ...
    let _ts = t[*COLS.get(&("ts".to_string())).unwrap()] as i32;
    for _sid in ENGIN_CONF.vsids.iter() {
        let ongoing = ONGOING_BID.read();
        for (_k, ci) in ongoing.iter() {
            // if ci.sidt != *sid {
            //     continue;
            ...
            for citem in ci.citems.iter() {
                ...
                info!(
                    "bid sidc={} [{} {}] bid_price={} rb={}",
                    citem.sidc, dinfo_m.bid1, dinfo_m.ask1, bid_price, rb
                );
        ...
        for (_k, ci) in ongoing.iter() {
            ...
            for citem in ci.citems.iter() {
                ...
                // info!("close fkey={}, pkey={} sidc={} rb={}", ci.from_key, pkey, citem.sidc, rb);
                ...
                info!(
                    "ask sidc={} [{} {}] ask_price={} rb={}",
                    citem.sidc, dinfo_m.bid1, dinfo_m.ask1, ask_price, rb
                );
                tds.push(MakeDecision {
                    create_ts: t[*COLS.get("ts").unwrap()] as i64,
                    client_order_id: client_order_id,
...
pub fn sample_onging(_t: &Vec<f64>, _tinfo: &mut TradeInfo) {
...
pub fn get_open_decision(t: &Vec<f64>, tinfo: &mut TradeInfo) -> Vec<MakeDecision> {
    let mut tds: Vec<MakeDecision> = Vec::new();
    let _max_pos_u = PAIRMM_CONF.amountu * 500.;
    let _max_ongoing = 5;
    ...
    for sid in ENGIN_CONF.vsids.iter() {
        ...
        let market_m = get_market()
            .get((e_m["exchange"].to_string() + ":" + &e_m["etype"] + ":" + &tinfo.symbol_std).as_str())
            .unwrap();
        ...
        let mid_m: f64 = (dinfo_m.bid1 + dinfo_m.ask1) / 2.;
        let _amount_hand_token = PAIRMM_CONF.amountu / contract_value_m / mid_m;
        let _upos_m: f64 = pos_m * contract_value_m * mid_m;
        let _gopenu = tinfo.open * mid_m;
        if !dinfo_m.is_finish_snap || mid_m == 0. {
            let warmup_left_m = snapshot_warmup_left(ts, dinfo_m, PAIRMM_CONF.open_snapshot_warmup_s);
            info!(
        ...
        for sidt in ENGIN_CONF.vsids.iter() {
            ...
            let market_t = get_market()
                .get(
                    (e_t["exchange"].to_string() + ":" + &e_t["etype"] + ":" + &tinfo.symbol_std)
                        .as_str(),
                )
                .unwrap();
            ...
            if !dinfo_t.is_finish_snap || mid_t == 0. {
                let warmup_left_t =
                    snapshot_warmup_left(ts, dinfo_t, PAIRMM_CONF.open_snapshot_warmup_s);
                info!(
            ...
            let _upos_t: f64 = pos_t * contract_value_t * mid_t;

            let _ranges_bids = get_range_of_pstat(t[0] as i64, "bid", *sid);
            let _ranges_asks = get_range_of_pstat(t[0] as i64, "ask", *sid);
            let _is_open = true;
...
pub fn write_pending_info(t: &Vec<f64>, tinfo: &mut TradeInfo) {
    let ts_t = t[*COLS.get(&("ts".to_string())).unwrap()] as i64;
