...
//                 let ranges_asks = get_range_of_pstat(t[0] as i64, "ask", *sid);
                ...
                for range in ranges_bid_prem {
                    ...
                    let ts = t[*COLS.get(&("ts".to_string())).unwrap()] as i64;
                    let fkey = "bid:".to_string()+&sid.to_string()+":"+&sidt.to_string()+":"+&range.to_string()+":"+&fdf.to_string()+":"+&ts.to_string();
                    let dupkey = "bid:".to_string()+&sid.to_string()+":"+&sidt.to_string()+":"+&range.to_string()+":"+&fdf.to_string();
                    let curr_cid = COID_INC.fetch_add(1, Ordering::Relaxed);
                    let client_order_id = "op_obid".to_string()+":"+&sid.to_string()+":"+&sidt.to_string()+"_"+&range.to_string()+"_"+&fdf.to_string()+"_"+ &curr_cid.to_string();
...
//                 let ranges_asks = get_range_of_pstat(t[0] as i64, "ask", *sid);
                ...
                for range in ranges_ask_prem {
                    ...
                    let fdf = t[*COLS.get(&fdf_key).unwrap()] as f64;
                    let ts = t[*COLS.get(&("ts".to_string())).unwrap()] as i64;
                    let fkey = "ask:".to_string()+&sid.to_string()+":"+&sidt.to_string()+":"+&range.to_string()+":"+&fdf.to_string()+":"+&ts.to_string();
                    let dupkey = "ask:".to_string()+&sid.to_string()+":"+&sidt.to_string()+":"+&range.to_string()+":"+&fdf.to_string();
                    let curr_cid = COID_INC.fetch_add(1, Ordering::Relaxed);
                    let client_order_id = "op_oask".to_string()+":"+&sid.to_string()+":"+&sidt.to_string()+"_"+&range.to_string()+"_"+&fdf.to_string()+"_"+ &curr_cid.to_string();
