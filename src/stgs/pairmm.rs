    ...
    if pitem.from_key.starts_with("cl_") {
        ...
        if pitem.side == "bid" {
            if let Some(mut ongoing) = ONGOING_BID.try_write_for(Duration::from_secs(1)) {
                if !ongoing.contains_key(&open_from_key.to_string()) {
                    info!(
                        "pkey={} not in the ongoing, cancel, oid={}",
                        open_from_key, pitem.client_order_id
                    );
                    return true;
                }
                ...
                if total_deal >= cvs.total_amount {
                    info!(
                        "pkey={} already finished[{} {}], cancel, oid={}",
                        open_from_key, total_deal, cvs.total_amount, pitem.client_order_id
                    );
                    return true;
                }
            }
...
pub fn get_close_decision(t: &Vec<f64>, tinfo: &mut TradeInfo) -> Vec<MakeDecision> {
    ...
    for sid in ENGIN_CONF.vsids.iter() {
        let _last_y: f64 = 60000.;
        let _last_rb_start: f64 = 0.003;
        let _last_rb_end: f64 = -0.003;
        ...
        for (_k, ci) in ongoing.iter() {
            ...
            for citem in ci.citems.iter() {
                ...
                info!(
                    "close fkey={}, pkey={} sidc={} rb={} client_order_id={} nasks={}",
                    ci.from_key, pkey, citem.sidc, rb, client_order_id, nasks
                );
    ...
    if ts % step == 0 {
        ...
        for sid in ENGIN_CONF.vsids.iter() {
            ...
            for sidt in ENGIN_CONF.vsids.iter() {
                ...
                info!(
                    "sidt={} amt={} cv={}",
                    sidt, amount_hand_token, contract_value_t
                );
                ...
                for range in ranges_bid_prem {
                    let fdf_key = "f_".to_string() + &sid.to_string() + "_" + &sidt.to_string();

                    let fdf = t[*COLS.get(&fdf_key).unwrap()] as f64;
                    ...
                    let _ru = RoundingStrategy::AwayFromZero;
                    ...
                    let mut tlenu = 0.0;
                    // 判断是否有值，并计算
                    if let Some(&value) = dinfo_t.bids.get(&OrderedFloat(bid_price_f64)) {
                        tlenu = bid_price_f64 * value;
                    }
                    if tlenu > PAIRMM_CONF.tlenu && fdf < PAIRMM_CONF.fdf {
                        tds.push(MakeDecision {
                            create_ts: t[*COLS.get("ts").unwrap()] as i64,
                            client_order_id: client_order_id,
            ...
            for sidt in ENGIN_CONF.vsids.iter() {
                ...
                for range in ranges_ask_prem {
                    ...
                    let tick_size = market_t.precision.tick_size;
                    let tick_size_keep = count_decimal_places(tick_size) as u32;
                    let _rd = RoundingStrategy::ToZero;
                    ...
                    let mut tlenu = 0.0;
                    // println!("============ask_price={},ask_price_f64={}==amount={:?}",ask_price,ask_price_f64,dinfo_t.asks.get(&OrderedFloat(ask_price_f64)));
                    // 判断是否有值，并计算
                    if let Some(&value) = dinfo_t.asks.get(&OrderedFloat(ask_price_f64)) {
                        tlenu = ask_price_f64 * value;
                    }
                    if tlenu > PAIRMM_CONF.tlenu && fdf * -1.0 < PAIRMM_CONF.fdf {
                        tds.push(MakeDecision {
                            create_ts: t[*COLS.get("ts").unwrap()] as i64,
                            client_order_id: client_order_id,
                            max_order_keep_s: PAIRMM_CONF.max_open_order_keep_s as i32,
...
pub fn make(t: &Vec<f64>, tinfo: &mut TradeInfo) -> Vec<MakeDecision> {
    let _pairpos_item_u = 50. * PAIRMM_CONF.amountu;
    let _max_pos_u = PAIRMM_CONF.amountu * 500.;
    let _max_pending_pair = 1;
    let _max_ongoing = 5;
    let _ts = t[0];
    let mut tds: Vec<MakeDecision> = Vec::new();
