...
lazy_static! {
    static ref COLS: HashMap<String, usize> = {
        ...
        map.insert("sb_0".to_string(), 3);
        map.insert("ss_0".to_string(), 4);
        
        map.insert("f_1".to_string(), 5);
        map.insert("s_1".to_string(), 6);
        map.insert("sb_1".to_string(), 7);
        map.insert("ss_1".to_string(), 8);
        
        map.insert("f_2".to_string(), 9);
        map.insert("s_2".to_string(), 10);
        map.insert("sb_2".to_string(), 11);
        map.insert("ss_2".to_string(), 12);
        
        map.insert("f_3".to_string(), 13);
        map.insert("s_3".to_string(), 14);
        map.insert("sb_3".to_string(), 15);
        map.insert("ss_3".to_string(), 16);
        
        map.insert("f_4".to_string(), 17);
        map.insert("s_4".to_string(), 18);
        map.insert("sb_4".to_string(), 19);
        map.insert("ss_4".to_string(), 20);
...
pub fn get_rbs_from_df(df:f64,  s:f64, side:&str, gopenu:f64, sli:f64, sid:&i32, upos:f64) ->Vec<f64> {
    let mut rs:Vec<f64> = Vec::new();
    
    //简单的线性逻辑
    let thr:f64 = 0.0003;
    let thr_t:f64 = 0.0001;
    let fee = &ARBMM2_CONF.fees[sid];
    let feet = fee["t"];
    ...
    if side == "buy" {
        let ft = (-df) - sli - feet;
        let mut p = false;
        if ft > 0.0003 && upos < 3000.{
            p = true
        }
        else if ft > 0.0002 && upos < 2000.{
            p = true
        }
        else if ft > 0.0001 && upos < 1000. {
            p = true
        }
...
pub fn make(t:&Vec<f64>, tinfo:&mut TradeInfo) -> Vec<MakeDecision>{
    ...
    for sid in ENGIN_CONF.vsids.iter() {
        ...
        let sli_buy = t[*COLS.get(&("sb_".to_string()+&sid.to_string())).unwrap()];
        let sli_sell = t[*COLS.get(&("ss_".to_string()+&sid.to_string())).unwrap()];
        
        
        let rb_bid_dfs = get_rbs_from_df(df, fstd, "buy", gopenu, sli_buy, sid, upos);
        let rb_ask_dfs = get_rbs_from_df(-df, fstd, "sell", gopenu, sli_sell, sid, upos);
        
        let mut buy_can_taker = false;
        let mut sell_can_taker = false;
        ...
        info!("buy_can_taker={} sell_can_taker={}", buy_can_taker, sell_can_taker);
        ...
        for rb_bid_df in &rb_bid_dfs {
            if *rb_bid_df == 1. {
                ...
                let client_order_id = "bid_t_".to_string()+&sid.to_string()+"_"+&rb_bid_df.to_string()+"_"+ &curr_cid.to_string();
                ...
                let mut bid_pass = false;
                if gopenu <= 0. || (gopenu > 0. && sell_can_taker) {
                    bid_pass = true;
                }
        ...
        for rb_ask_df in &rb_ask_dfs {
            if *rb_ask_df == 1. {
                ...
                let amount_token = amount_hand_token;
                let curr_cid = COID_INC.fetch_add(1, Ordering::Relaxed);
                let client_order_id = "ask_t_".to_string()+&sid.to_string()+"_"+&rb_ask_df.to_string()+"_"+ &curr_cid.to_string();
                tds.push(MakeDecision{create_ts:t[*COLS.get("ts").unwrap()] as i64,
                    client_order_id:client_order_id, 
                    max_order_keep_s:0, 
                    side:"sell".to_string(), 
                ...
                let mut ask_pass = false;
                if gopenu >= 0. || (gopenu < 0. && buy_can_taker) {
                    ask_pass = true;
                }
