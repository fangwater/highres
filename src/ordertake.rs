...
use crate::trade::{DepthInfo, MakeDecision, TradeInfo};
use log::info;
...
pub fn add_taking(tinfo: &mut TradeInfo, tds: &MakeDecision) {
    ...
    if tds.side == "buy".to_string() {
        ...
        for (price, amount) in dinfo.asks.iter() {
            ...
            if amount > &left_amount {
                ...
                *p += left_amount;
                ...
                info!(
                    "finish taker-buy, deal_amount={} total_deal_avg_price={}",
                    deal_amount, total_deal_avg_price
                );
                break;
            } else {
        ...
        if total_deal_token == 0. {
            ...
            info!(
                "finish taker-buy(nol2offered), deal_amount={} total_deal_avg_price={}",
                deal_amount, total_deal_avg_price
            );
        }
        ...
        for (price, amount) in dinfo.bids.iter().rev() {
            ...
            if amount > &left_amount {
                ...
                *p -= left_amount;
                ...
                info!(
                    "finish taker-sell, deal_amount={} total_deal_avg_price={} amount_f64={}",
                    deal_amount, total_deal_avg_price, amount_f64
                );
                break;
            } else {
                let qc = amount * price.into_inner();
