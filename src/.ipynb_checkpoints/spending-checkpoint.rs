...
pub fn drop_pending(ts:i64, tinfo:&mut TradeInfo, cds:&CancelDecision) {
    ...
    if &cds.side == "bid" {
        ...
        if let Some(mut porders) = S_PENDING_PRICEKEY_ASKS[&cds.sid].try_write_for(Duration::from_secs(1)) {
            match porders.phash.get_mut(&OrderedFloat(cds.price)) {
                Some(vs) => {
                    let mut idx2del:usize = 0;
                    for v in &mut *vs {
...
pub fn add_pending(tinfo:&mut TradeInfo, tds:&MakeDecision) {
    ...
    if tds.side == "buy".to_string() {
        ...
        if let Some(mut porders) = S_PENDING_PRICEKEY_BIDS[&tds.sid].try_write_for(Duration::from_secs(1)) {
        ...
        if ENGIN_CONF.is_spending_open_dump {
            ...
            let rd:RecordDumpItem = RecordDumpItem{
                ...
                update_ts:tds.create_ts*1000,
                client_order_id:p.client_order_id.to_string(),
                symbol:tinfo.symbol.to_string(),                 ttype:"maker".to_string(),                 sid:tds.sid, 
                side:"sell".to_string(), 
                price:p.price, 
                amount_init:tds.amount, 
                amount_update:0., 
        ...
        if let Some(mut porders) = S_PENDING_PRICEKEY_ASKS[&tds.sid].try_write_for(Duration::from_secs(1)) {
