        ...
        for (_sid, sinfo) in ENGIN_CONF.sids.iter() {
            ...
            if let Ok(mut markets) = fetch_markets(&sinfo["exchange"], mt) {
                for market in markets.iter_mut() {
                    ...
                    let symbol_std = symbol_plat2std(&market.symbol, sinfo["exchange"].as_str(), sinfo["etype"].as_str());                
                    let exchange_info_key = sinfo["exchange"].to_string() + ":" + &sinfo["etype"]  + ":" + &symbol_std;
                            ...
                            if let Some(filters) = market_rc.info["filters"].as_array(){
                                for filter in filters.iter(){
                                    if filter["filterType"] == "PRICE_FILTER"{
                                        let tick_size = filter["tickSize"].as_str().unwrap().parse::<f64>().unwrap();  
