...
pub fn deal_order(ts:i64, sid:&i32, tinfo:&mut TradeInfo, pitem:&PendingItem, side: &str, client_order_id: &str, _price:f64, amount:f64, amount_traded: f64, make_amout:f64, is_full_filled: bool) {
    ...
    let rd:RecordDumpItem = RecordDumpItem{create_ts:pitem.create_ts,update_ts:update_ts_ms,client_order_id:pitem.client_order_id.to_string(),symbol:tinfo.symbol.to_string(), ttype:"maker".to_string(), sid:*sid, side:side.to_string(), price:pitem.price, amount_init:pitem.amount_init*contract_value, amount_update:make_amout*contract_value, status:status, inpos:pitem.inpos, tlen:pitem.tlen, from_key:pitem.from_key.to_string(), bid1:dinfo.bid1, ask1:dinfo.ask1};
    write_to_csv(&rd);
    let p = tinfo.pos.get(sid).unwrap();
    let tsrd:TSRecordItem = TSRecordItem{create_ts:update_ts_ms, symbol:tinfo.symbol.to_string(), sid:*sid, pos:*p, open:tinfo.open};
                ...
                if price_trade >= price.into_inner() {
                    ...
                    for pitem in vpitem.iter_mut() {
                        ...
                        if taker_amount <= 0. {
                            ...
                            pitem.trade_comsume_amount = pitem.trade_comsume_amount + amount_trade - taker_amount;
                            //部分成交
                            deal_order(ts, &sid, tinfo, pitem, "sell", &pitem.client_order_id, pitem.price, pitem.amount, pitem.amount_init - pitem.amount, taker_amount, false);
                            
                        } else if taker_amount > pitem.amount {
                            //deal_order(&exchange, &etype , &symbol_std, &pitem.account_id, &pitem.client_order_id, pitem.amount_init,pitem.amount, "sell",true);
                            
                            deal_order(ts, &sid, tinfo, pitem, "sell", &pitem.client_order_id, pitem.price, pitem.amount, pitem.amount_init, pitem.amount, true);
                            pitem.amount = 0.;
                            ...
                            panic!("taker amount[{}] > amount_left[{}]", taker_amount, pitem.amount);
                        }
                        cid+=1;
                    }
        ...
        if let Some(mut porders) = S_PENDING_PRICEKEY_BIDS[&sid].try_write_for(Duration::from_secs(1)) {
            ...
            for (price, vpitem) in porders.phash.iter_mut() {
                if price_trade <= price.into_inner() {
                    ...
                    let mut vitems:Vec<usize> = Vec::new();
                    for pitem in vpitem.iter_mut() {
                        info!("ask coid={} amount={} price={}", &pitem.client_order_id, amount_trade, price_trade);
                        ...
                        if taker_amount <= 0. {
                            ...
                            pitem.trade_comsume_amount = pitem.trade_comsume_amount + amount_trade - taker_amount;
                            deal_order(ts, &sid, tinfo, pitem, "buy", &pitem.client_order_id, pitem.price, pitem.amount, pitem.amount_init - pitem.amount,taker_amount, false);


                        } else if taker_amount > pitem.amount {
                            deal_order(ts, &sid, tinfo, pitem, "buy", &pitem.client_order_id, pitem.price, pitem.amount, pitem.amount_init, pitem.amount, true);
