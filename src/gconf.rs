...
lazy_static! {
    pub static ref ENGIN_CONF: EnginConf = {
        ...
        let econf: EnginConf = load_config_aenginconf(&config_file).unwrap();
        econf
    };
    pub static ref ARBMM_CONF: ArbmmConf = {
        let config_file = highres_config_path();
        let econf: ArbmmConf = load_config_arbmmconf(&config_file).unwrap();
        econf
    };
    pub static ref PAIRMM_CONF: PairmmConf = {
        let config_file = highres_config_path();
        let econf: PairmmConf = load_config_pairmmconf(&config_file).unwrap();
        econf
    };
    pub static ref ARBMM2_CONF: Arbmm2Conf = {
        let config_file = highres_config_path();
        let econf: Arbmm2Conf = load_config_arbmm2conf(&config_file).unwrap();
        econf
    };
    pub static ref ARBMTSAMPLING_CONF: ArbmtSamplingConf = {
        let config_file = highres_config_path();
        let econf: ArbmtSamplingConf = load_config_arbmmsamplingconf(&config_file).unwrap();
        econf
    };
    pub static ref NMT_CONF: NmtConf = {
        let config_file = highres_config_path();
        let econf: NmtConf = load_config_nmtconf(&config_file).unwrap();
        econf
    };
}
