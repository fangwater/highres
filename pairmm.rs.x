...
use crate::record::{write_to_csv_orstatus,PendingStatus};
...
pub fn get_range_of_pstat(ts:i64, side:&str, sid:i32)->Vec<f64> {
    ...
    let mut vs:Vec<f64> = vec![0.0,0.0002, 0.0005,0.001];
    return vs;
}
...
pub fn get_close_decision(t:&Vec<f64>, tinfo:&mut TradeInfo) -> Vec<MakeDecision> {
    ...
    let ts = t[*COLS.get(&("ts".to_string())).unwrap()] as i32;
    for sid in ENGIN_CONF.vsids.iter() {
        ...
        for sid in ENGIN_CONF.vsids.iter() {
            ...
            for sidt in ENGIN_CONF.vsids.iter() {
                ...
                for range in ranges_bid_prem {
                    ...
                    let curr_cid = COID_INC.fetch_add(1, Ordering::Relaxed);
                    let client_order_id = "op_obid".to_string()+":"+&sid.to_string()+":"+&sidt.to_string()+"_"+&range.to_string()+"_"+ &curr_cid.to_string();
            ...
            for sidt in ENGIN_CONF.vsids.iter() {
                ...
                for range in ranges_ask_prem {
                    ...
                    let curr_cid = COID_INC.fetch_add(1, Ordering::Relaxed);
                    let client_order_id = "op_oask".to_string()+":"+&sid.to_string()+":"+&sidt.to_string()+"_"+&range.to_string()+"_"+ &curr_cid.to_string();
        ...
        if let Some(porders) = S_PENDING_PRICEKEY_BIDS[&sid].try_write_for(Duration::from_secs(1)) {
            for (_price, vpitem) in porders.phash.iter() {
                for pitem in vpitem.iter() {
                    ...
                    let ps:PendingStatus = PendingStatus{symbol:tinfo.symbol.to_string(),create_ts:pitem.create_ts,ts:ts_t, client_order_id:pitem.client_order_id.clone(), from_key:pitem.from_key.clone(), amount_init:pitem.amount_init,status:st,filled_amount:filled_am,inpos:pitem.inpos,backlen:pitem.backlen,tlen:pitem.tlen};
                    // write_to_csv_orstatus(&ps); 
                }
            }
        }
