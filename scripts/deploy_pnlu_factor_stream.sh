...
usage() {
  cat <<'EOF'
Usage:
  deploy_pnlu_factor_stream.sh [--host <user@ip>] [--dir <path>]
...
Examples:
  bash scripts/deploy_pnlu_factor_stream.sh
  bash scripts/deploy_pnlu_factor_stream.sh --host u171@10.61.10.32
  bash scripts/deploy_pnlu_factor_stream.sh --dir "/home/u171/mth_pub"
EOF
}
...
echo "[INFO] Example: ssh $TARGET_HOST \"cd $TARGET_DIR && ./scripts/start_pnlu_factor_stream.sh\""
