...
struct MarketSpotV5 {
    ...
    lot_size_filter: LotSizeFilterSpotV5,
    #[serde(rename = "priceFilter")]
    price_filter: PriceFilterSpotV5,
}

#[derive(Serialize, Deserialize, Debug)]
struct LotSizeFilterSpotV5 {
...
struct ResultSpotV5 {
    category: String,
    list: Vec<MarketSpotV5>,
}

#[derive(Serialize, Deserialize, Debug)]
struct ResponseSpotV5 {
...
fn fetch_markets_raw_spot() -> Result<Vec<MarketSpotV5>> {
    let txt = http_get("https://api.bybit.com/v5/market/instruments-info?category=spot", None)?;
    let resp = serde_json::from_str::<ResponseSpotV5>(&txt)?;
    assert_eq!(resp.ret_code, 0);
    Ok(resp.result.list.into_iter().filter(|m| m.status == "Trading").collect())
}
...
fn to_market_spot(raw_market: &MarketSpotV5) -> Market {
    let pair = crypto_pair::normalize_pair(&raw_market.symbol, "bybit").unwrap();
    let (base, quote) = {
        let v: Vec<&str> = pair.split('/').collect();
        (v[0].to_string(), v[1].to_string())
    };
    
    let delivery_date: Option<u64> = Some(0);
   
    Market {
        exchange: "bybit".to_string(),
        market_type:MarketType::Spot,
        symbol: raw_market.symbol.to_string(),
        base_id: raw_market.base_coin.to_string(),
        quote_id: raw_market.quote_coin.to_string(),
        settle_id: if raw_market.quote_coin == "USDT" {
            Some(raw_market.quote_coin.to_string())
        } else {
            Some(raw_market.base_coin.to_string())
        },
        base,
        quote,
        settle: if raw_market.quote_coin == "USDT" {
            Some(raw_market.quote_coin.to_string())
        } else {
            Some(raw_market.base_coin.to_string())
        },
        ...
        precision: Precision {
            tick_size: raw_market.price_filter.tick_size.parse::<f64>().unwrap(),
            lot_size: raw_market.lot_size_filter.min_order_qty.parse::<f64>().unwrap(),
        },
        quantity_limit: Some(QuantityLimit {
            min: Some(raw_market.lot_size_filter.min_order_qty.parse::<f64>().unwrap()),
            max: Some(raw_market.lot_size_filter.max_order_qty.parse::<f64>().unwrap()),
            notional_min: None,
            notional_max: None,
        }),
        contract_value: Some(1.0),
...
fn fetch_spot_markets() -> Result<Vec<Market>> {
    let markets = fetch_markets_raw_spot()?
        .into_iter()
        .filter(|m| m.quote_coin == "USDT")
        .map(|m| to_market_spot(&m))
        .collect::<Vec<Market>>();
    Ok(markets)
}